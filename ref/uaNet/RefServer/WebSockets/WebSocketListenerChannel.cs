...
/// </summary>
internal class WebSocketListenerChannel : UaSCUaBinaryChannel
{

    /// <summary>
    /// The channel request event handler.
    /// </summary>
    protected WebSocketChannelRequestEventHandler RequestReceived { get; private set; }

    /// <summary>
    /// The report open secure channel audit event handler.
    /// </summary>
    protected WebSocketReportAuditOpenSecureChannelEventHandler ReportAuditOpenSecureChannelEvent { get; private set; }

    /// <summary>
    /// The report close secure channel audit event handler.
    /// </summary>
    protected WebSocketReportAuditCloseSecureChannelEventHandler ReportAuditCloseSecureChannelEvent { get; private set; }

    /// <summary>
    /// The report certificate audit event handler.
    /// </summary>
    protected WebSocketReportAuditCertificate ReportAuditCertificateEvent { get; private set; }

    private readonly ILogger _logger;
    private bool _responseRequired;
    private uint _lastTokenId;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
        ...
        : base(
            ...
            telemetry)
    {
        _logger = telemetry.CreateLogger<WebSocketListenerChannel>();
        Listener = listener;
    }
    ...
    /// </summary>
    public virtual string ChannelName => "WEBSOCKETLISTENERCHANNEL";
    ...
    /// </summary>
    public void SetRequestReceivedCallback(WebSocketChannelRequestEventHandler callback)
    {
        lock (DataLock)
        {
    ...
    public void SetReportOpenSecureChannelAuditCallback(
        WebSocketReportAuditOpenSecureChannelEventHandler callback)
    {
        lock (DataLock)
        {
    ...
    public void SetReportCloseSecureChannelAuditCallback(
        WebSocketReportAuditCloseSecureChannelEventHandler callback)
    {
        lock (DataLock)
        {
    ...
    /// Sets the callback used to raise channel audit events.
    /// </summary>
    public void SetReportCertificateAuditCallback(WebSocketReportAuditCertificate callback)
    {
        lock (DataLock)
        {
    ...
    public IMessageSocket GetSocket()
    {
        return Socket;
    }
    ...
    public void Attach(uint channelId, WebSocket socket)
    {
        _logger.LogDebug(
            "{Channel} ATTACHING SOCKET: {SocketHandle:X8}, ChannelId={ChannelId}",
            ChannelName,
            socket != null ? socket.GetHashCode() : 0,
            channelId);
        ...
        lock (DataLock)
        {
            ...
            Socket = new WebSocketMessageSocket(socket, this, BufferManager, Quotas.MaxBufferSize, Telemetry);
    ...
    protected void ForceChannelFault(ServiceResult reason)
    {
        lock (DataLock)
        {
            ...
            if (Socket != null && _responseRequired)
            {
            ...
            if (close)
            {
                ...
                // {
                //     var wsTransportListener = Listener as WebSocketTransportListener;
                //     wsTransportListener?.MarkAsPotentialProblematic(
    ...
    {
        _logger.LogTrace("ChannelId {ChannelId}: SendErrorMessage={Status}", ChannelId, error.StatusCode);
    ...
    {
        return Utils.IncrementIdentifier(ref _lastTokenId);
    }
}
...
/// </summary>
internal delegate void WebSocketChannelRequestEventHandler(
...
/// </summary>
internal delegate void WebSocketChannelStatusEventHandler(
...
/// </summary>
internal delegate void WebSocketReportAuditOpenSecureChannelEventHandler(
    WebSocketServerChannel channel,
    OpenSecureChannelRequest request,
    X509Certificate2 clientCertificate,
    Exception exception);
...
/// </summary>
internal delegate void WebSocketReportAuditCloseSecureChannelEventHandler(
    WebSocketServerChannel channel,
    Exception exception);
...
/// Used to report an open secure channel audit event.
/// </summary>
public delegate void WebSocketReportAuditCertificate(
    X509Certificate2 clientCertificate,
    Exception exception);
