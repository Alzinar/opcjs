...
#pragma warning disable CS8620 // Nullability mismatch in delegate/task argument.
#pragma warning disable CS8622 // Nullability mismatch in delegate parameter.
#pragma warning disable CS8625 // Cannot convert null literal to non-nullable reference type.
#pragma warning disable CA2000 // OPC UA token/certificate objects have ownership-transfer semantics not tracked by the analyzer.
...
/// </summary>
internal sealed class WebSocketServerChannel : WebSocketListenerChannel
{
    private readonly ILogger _logger;
    private SortedDictionary<uint, IServiceResponse> _queuedResponses;
    private ReverseConnectAsyncResult _pendingReverseHello;

    private static readonly string _implementationString =
        ".NET Standard ServerChannel UA-WS " + Utils.GetAssemblyBuildNumber();
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
        ...
        : base(
            ...
            telemetry)
    {
        _logger = telemetry.CreateLogger<WebSocketServerChannel>();
        _queuedResponses = [];
    }
    ...
    /// </summary>
    public override string ChannelName => "WEBSOCKETSERVERCHANNEL";
    ...
    private sealed class ReverseConnectAsyncResult : AsyncResultBase
    {
#pragma warning disable CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
#pragma warning disable IDE0290 // Use primary constructor
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
        ...
        int timeout)
    {
        ...
        var ar = new ReverseConnectAsyncResult(callback, callbackData, timeout, _logger);

        var wsMessageSocketFactory = new WebSocketMessageSocketFactory(Telemetry);
        ar.Socket = Socket = wsMessageSocketFactory.Create(
            this,
            BufferManager,
            ReceiveBufferSize);
    ...
    private void OnReverseConnectComplete(object sender, IMessageSocketAsyncEventArgs result)
    {
        ...
        if (ar == null || _pendingReverseHello != null)
        {
            return;
        }
    ...
    {
        _logger.LogDebug(
            "ChannelId {Id}: HandleIncomingMessage called with MessageType {MessageType:X8}",
            ChannelId,
            messageType);
        lock (DataLock)
        {
    ...
    private void OnChannelReconnected(object state)
    {
        ...
        foreach (KeyValuePair<uint, IServiceResponse> response in responses)
        {
            ...
            {
                _logger.LogError(e, "Unexpected error re-sending request (ID={Id}).", response.Key);
            }
        }
    }
        ...
        ArraySegment<byte> messageChunk)
    {
        ...
        try
        {
            ...
            if (requestType == SecurityTokenRequestType.Issue)
            {
                _logger.SecureChannelCreated(
                    _implementationString,
            ...
            {
                _logger.SecureChannelRenewed(_implementationString, Utils.Format("{0}", ChannelId));
            }
    ...
    protected override void NotifyMonitors(ServiceResult status, bool closed)
    {
        ...
        {
            _logger.LogError(e, "Error raising StatusChanged event.");
        }
    }
    ...
    {
        ReverseConnectAsyncResult ar = _pendingReverseHello;
        if (ar != null &&
            ar == Interlocked.CompareExchange(ref _pendingReverseHello, null, ar))
        {
            ar.Exception = e;
            ar.OperationCompleted();
        }
    ...
    private void SendOpenSecureChannelResponse(
        ...
        OpenSecureChannelRequest request)
    {
        _logger.LogTrace("ChannelId {Id}: SendOpenSecureChannelResponse()", ChannelId);
        ...
        ArraySegment<byte> messageChunk)
    {
        ...
        catch (Exception e)
        {
            ...
            _logger.LogError(e, "Unexpected error processing CloseSecureChannel request.");
        }
        finally
        {
    ...
    private bool ProcessRequestMessage(uint messageType, ArraySegment<byte> messageChunk)
    {
        ...
        try
        {
            messageBody = ReadSymmetricMessage(
                messageChunk,
                true,
#pragma warning disable CA2000 // token ownership transferred into channel state (CurrentToken / PreviousToken).
                out token,
#pragma warning restore CA2000
        ...
        {
            _logger.LogInformation("Channel {Id}: full -- delay processing.", Id);
            ...
            {
                _logger.LogWarning("Channel {Id}: break socket connection.", Id);
                ChannelClosed();
                return false;
            }
        }
        ...
        try
        {
            ...
            // decode the request.
            using var arraySegmentStream = new ArraySegmentStream(chunksToProcess);
            if (BinaryDecoder.DecodeMessage(
                    arraySegmentStream,
                    null,
                    Quotas.MessageContext)
        ...
        {
            _logger.LogError(e, "Unexpected error processing request.");
            SendServiceFault(
                token,
                requestId,
                ServiceResult.Create(
    ...
    {
        _logger.LogDebug(
            "ChannelId {ChannelId}: SendResponse called for RequestId {RequestId}",
            ChannelId,
            requestId);
        ...
        lock (DataLock)
        {
            ...
            {
                _queuedResponses[requestId] = response;
                return;
            }
            ...
            //Opc.Ua.Utils.EventLog.SendResponse((int)ChannelId, (int)requestId);
            _logger.LogTrace(
    ...
    {
        Task.Factory.StartNew(action, _queuedResponses);
        _queuedResponses = [];
    }
