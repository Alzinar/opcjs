...
cleanup() {
    ...
    pkill -9 -f "opcjs/RefServer/dist/index\.js" 2>/dev/null || true
    rm -rf "$REPO_ROOT/tmp"
}
trap cleanup EXIT

echo "[test:ref] Building opcjs-base / opcjs-client / opcjs-server (skipped by Nx if already up to date)..."
(cd "$REPO_ROOT" && npx nx run-many -t build -p opcjs-base,opcjs-client,opcjs-server) || exit 1
